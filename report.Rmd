...
We will be using the Ames, Iowa individual residential property sales data set freely available on Kaggle.com.  The data set contains 2,930 observations with 79 explanatory variables. All observations occur between 2006 and 2010. It is important to remember that any conclusions drawn from this data can only be applied in Ames, Iowa. For more information on the data, or to download it yourself, visit https://www.kaggle.com/c/house-prices-advanced-regression-techniques.
...
We also included the Sale Condition variable in all three models because the context of the sale seems like way too key of a factor to leave out of any model that is meant to be easily understood
...
![Figure Ma. Model Performance Statistics](images/ms_model1.jpg)
...
![Figure Mb. Model Performance Statistics](images/ms_model2.jpg)
...
![Figure Mc. Model Performance Statistics](images/ms_model3.jpg)
...
![Figure Md. Model Performance Statistics](images/ms_model4.jpg)

### Model Selection: Interaction Terms and Indicator Variables

With outliers removed, the model is once again studied for opportunities to optimize model fit.  One thing that has not been considered so far are interactions between any predictors that can explain additional variability or offer more flexible parameterizations.

There are a few variables with strong linear correlations in the training data set. `GarageCars/Garage Area`, `TotRmsAbvGrd`, and `GrLivArea` will be variables that probably should not have interactions included in the model if the correlated terms are present.  A table of all Pearson correlations greater than .5 is shown in Appendix D, Figure 8.  These were considered significant enough to be mindful of when selecting interactions.


A first attempt to discover interactions was made by regressing partial residuals from Model 4 on all possible interaction combinations using `PROC GLMSELECT`.  However, this yielded no useful results because no terms were identified. 
...
![Figure Me. Model Performance Statistics](images/ms_model5.jpg)

Two indicator terms are included to allow increased flexibility in the parameterizations of the `BsmtFinSF1*idxhasFB1` and `WoodDeckSF*idxhasWD` interactions.  This essentially allows for homes without these features to be modeled with a separate parameter so that the polynomial fits represent homes that actually have the feature.

![Figure Mf. Model Performance Statistics](images/ms_model6.jpg)

Finally, YearBuilt was converted into a piecewise interaction with an old vs new indicator so that we can allow for differing polynomial regression slopes based on whether homes are built after 2000.  This represents the final set of predictors, shown in model 7.  This model also includes an optimized regrouping of the MSSubClass variable.

![Figure Mg. Model Performance Statistics](images/ms_model7.jpg)
...
![Figure 9. Final Kaggle Score](images/kaggle.jpg)
...
Figure 2. Predictors used in assumption validation model

![Figure 2. Predictors used in assumption validation model](images/modelterms_assumptions.jpg)

Figure 6. Model Predictors and Associated Scores

![](images/ms_fa_comb1.jpg) ![](images/ms_fa_comb2.jpg)

Figure 7. Outliers Removed

![Figure 7. Outliers Removed](images/outlier_list.jpg)

Figure 8. Linear Correlations in Training Data Sample
![Figure 8. Linear Correlations in Training Data Sample](images/correlations.jpg)
